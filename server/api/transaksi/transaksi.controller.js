...
module.exports = {
    ...
    controller_update: function (req, res) {
        ...
        update(data, (err, result) => {
            if (!result[0]) {
                return response_format(res, 0, `Couldn't find transaksi with id ${id}`).status(404)
            }
    ...
    controller_delete: function (req, res) {
        del(req.body.id, (err, result) => {
            if (!result) {
                return response_format(res, 0, `Couldn't find transaksi with id ${req.body.id}`).status(404)
            }
            else return service_callback(err, result, res)
        })
    }
